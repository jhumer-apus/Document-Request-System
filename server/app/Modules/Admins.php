...
use App\Models\AdminProfilePicture;
...
class Admins
{
    ...
    public function store($payload)
    {
        ...
        $userTransaction = Admin::create($data);

        //generate code for verification
        $emailCode = $this->generateCode(). $userTransaction->id;
        $storeCodeTransaction = Admin::where('id', $userTransaction->id)->update(['email_code'=> $emailCode]);
    ...
    public function verify($payload){
        $token = $payload->token;
        $userTransaction = Admin::where('email_code', $token)
    ...
    public function checkEmail($email){
        ...
        $isEmailExist = Admin::where('email', $email)->first();
        if(!$isEmailExist){
            return false;
        }
        return $isEmailExist;
    ...
    public function updatePassword($payload){
        ...
        $updateTransaction= Admin::where('id',$userId)->update($data);
    ...
    public function uploadPhoto($payload){
        $getUserProfilePic = AdminProfilePicture::where('admin_id',Auth::user()->id)->first();

        $userProfilePicture = new AdminProfilePicture;  
        ...
        if($getUserProfilePic){
            AdminProfilePicture::where('admin_id',Auth::user()->id)
                                ->update([
                                    'filename'=>$file_name,
                                    'path'=>$file_path
                                ]);
        }else{
            $userProfilePicture->admin_id = Auth::user()->id;
    ...
    public function changePassword($payload){
        ...
        $userTransaction = Admin::where('id', $id)->update(['password'=> Hash::make($newPassword)]);
